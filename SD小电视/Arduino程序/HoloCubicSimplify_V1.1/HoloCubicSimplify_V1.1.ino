 ...
 * 
 * 作       者：Misaka
 * 讨 论 QQ 群：811058758               //暗号HCS
 * 创 建 日 期：2021.04.19
 * 最后更改日期：2021.07.04
 * 适合硬件版本：HoloCubicSimplify       //群内有相关文件
 * 
 * 软 件 版 本：V1.1
 * 软件更新日志：210704 修复了联网进度条等若干Bug，增加了微信联网功能
 * 
 * *****************************************************************/
...
String cityCode = "101080706";  //天气城市代码 长沙:101250101株洲:101250301衡阳:101250401
int tempnum = 0;   //温度百分比
int huminum = 0;   //湿度百分比
int tempcol =0xffff;
int humicol =0xffff;
...
void SmartConfig(void)//微信配网
{
  ...
  TJpgDec.drawJpg(0,0,qr, sizeof(qr));//显示二维码
...
void setup()
{
  ...
  duty = constrain(0.1, 0, 0.1);
  duty = 1 - duty;
  ledcWrite(LCD_BL_PWM_CHANNEL, (int)(duty * 255));
  tft.begin(); /* TFT init */
  tft.setRotation(4); /* mirror  屏幕方向 4为镜像  6为正向*/
